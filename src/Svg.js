...
import {formatPropsAsNumber, getObjectString, stringToBoolean} from './Format.js'
    ...
    function addSVGZoomListener(svg, options = {zoom_functions: []}){//{f: zoom: boundary:}
  ...
  function runZoomFunctions(svg, functions){
    ...
    for(let settings of functions){
        ...
        if(boundary == '>' && Number(svg.dataset.zoom) > zoom){

            f(svg);
        }
        else if(boundary == '<' && Number(svg.dataset.zoom) < zoom){
  ...
  `
    ...
    function addSVGTranslateListeners(svg){
